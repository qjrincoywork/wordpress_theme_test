...
function ubermenu_get_settings_fields_instance( $config_id ){
	...
	$settings = array(
		...
		array(
			...
			'type'	=> 'select',
			'desc'	=> __( 'If you disable the skin, you must provide your own custom skin.  Otherwise, use the Minimal Base or Vanilla skin as a Customizer base.' , 'ubermenu' ). ' <br/> <a target="_blank" href="https://sevenspark.com/goods/category/ubermenu-skin-packs?src=plugin">'.__( 'Get more skins', 'ubermenu' ).'</a>',
			//'options'	=> array(),
		...
		80 => array(
			...
			'type'	=> 'radio',
			'desc'	=> __( 'Orient the menu vertically or horizontally' , 'ubermenu' ) . '<br/><a target="_blank" href="http://goo.gl/UaAb2z">'.__( 'Vertical Menu Demo', 'ubermenu' ).'</a>',
			'options'=> array(
		...
		110 => array(
			...
			'options'=> array(
				//'auto'		=> __( 'Automatic' , 'ubermenu' ),
				'hover' 		=> [
					'title' => __( 'Hover', 'ubermenu' ),
					'desc' => __('The submenu will open immediately when the mouse enters the menu item', 'ubermenu'),
				],
				'hover_intent' 	=> [
					'title' => __( 'Hover Intent', 'ubermenu' ),
					'desc'	=> __( 'The submenu will open when the mouse lingers on an item', 'ubermenu' ),
				],
				'click'			=> [
					'title' => __( 'Click', 'ubermenu' ),
					'desc' => __('The submenu will open when an item is clicked', 'ubermenu' ),
				],
			),
		...
		401 => array(
			...
			'desc' 		=> __( 'Choose how the menu should be displayed on mobile.', 'ubermenu' ),
			'type' 		=> 'radio',
			'options'	=> [
				'inline'	=> [
					'title' => __( 'Inline' , 'ubermenu' ),
					'desc' => __('The mobile menu will display within the natural flow of the layout, which will be controlled by the theme.', 'ubermenu'),
				],
				'modal'		=> [
					'title' => __( 'Modal' , 'ubermenu' ),
					'desc' => __('The menu will be displayed in a full-screen, fixed position overlay', 'ubermenu'),
				],
			],
			'default'	=> 'inline',
			'group'		=> 'responsive',
		...
		430 => array(
			...
			'desc' 		=> __( 'The text to display on the responsive toggle.  {{menu_title}} can be used to display the title of the toggled menu.', 'ubermenu' ),
			'type' 		=> 'text',
		...
		431 => array(
			'name' 		=> 'responsive_toggle_aria_label',
			'label' 	=> __( 'Responsive Toggle ARIA Label', 'ubermenu' ),
			'desc' 		=> __( 'ARIA Label for the responsive toggle button.  If you leave the Toggle Content empty, this should be set for accessibility.  {{menu_title}} can be used to display the title of the toggled menu.', 'ubermenu' ),
			'type' 		=> 'text',
			'default' 	=> sprintf(__( 'Toggle %s', 'ubermenu' ), '{{menu_title}}'),
			'group'		=> 'responsive',
			// 'sanitize_callback' => 'ubermenu_allow_html',
			'customizer'	=> true,
			'customizer_section' => 'toggle',
		...
		438 => array(
			'name' 		=> 'responsive_toggle_placement',
			'label' 	=> __( 'Responsive Toggle Placement', 'ubermenu' ),
			'desc' 		=> __( 'Choose where to inject the Responsive Toggle (if enabled above).  The Fixed option can be useful when paired with the Modal responsive menu display, if your theme hides the menu and toggle by default on mobile.', 'ubermenu' ),
			'type' 		=> 'radio',
			'options'	=> [
				'inline'	=> [
					'title' => __( 'Inline' , 'ubermenu' ),
					'desc' => __('The toggle will be inserted just before the menu within the site layout.','ubermenu'),
				],
				'fixed'		=> [
					'title' => __( 'Fixed' , 'ubermenu' ),
					'desc' => __('The toggle will be injected into the footer, and then set in a fixed position via CSS.  This option may overlap other content on your site.','ubermenu'),
				],
			],
			'default' 	=> 'inline',
			'group'		=> 'responsive',
		...
		451 => array(
			...
			'desc' 		=> __( 'Determine how the mobile submenus should act.', 'ubermenu' ),
			'type' 		=> 'radio',
			'options'	=> [
				'dropdown'		=> [
					'title' => __( 'Dropdown' , 'ubermenu' ),
					'desc' => __('Dropdown submenus will overlay other content in the menu.', 'ubermenu' ),
				],
				'accordion'		=> [
					'title' => __( 'Accordion' , 'ubermenu' ),
					'desc' => __('Accordions will push down content below them.  Accordion mode will only apply when interactions are via click or tap, not hover.', 'ubermenu' ),
				],
			],
			'default'	=> 'dropdown',
			'group'		=> array( 'responsive' , 'submenus' ),
		...
		452 => array(
			'name' 		=> 'mobile_accordion_folding',
			'label' 	=> __( 'Mobile Accordion Submenu Folding', 'ubermenu' ),
			'desc' 		=> __( 'Choose if more than one submenu can be open at a time with accordion submenus', 'ubermenu' ),
			'type' 		=> 'radio',
			'options'	=> [
				'multiple'		=> [
					'title' => __( 'Multiple' , 'ubermenu' ),
					'desc' => __('Multiple submenus can be open at the same time.','ubermenu' ),
				],
				'single'		=> [
					'title' => __( 'Single' , 'ubermenu' ),
					'desc' => __('Only one submenu can be open at a time.  When a new submenu is opened, other opened submenus at the same level will be closed.','ubermenu' ),
				],
			],
			'default'	=> 'multiple',
			'group'		=> array( 'responsive' , 'submenus' ),
		...
		453 => array(
			'name' 		=> 'mobile_accordion_indent',
			'label' 	=> __( 'Accordion Dropdown Indentation', 'ubermenu' ),
			'desc' 		=> __( 'Indent inner dropdowns within the accordion by this value (first level of submenus will remain aligned with top level items).  20px works well if you\'ve left the default menu spacing.  Leave blank for no indentation.', 'ubermenu' ),
			'type' 		=> 'text',
			'default'	=> '',
			'group'		=> array( 'responsive' , 'submenus' ),
			'customizer'	=> true,
			'customizer_section' => 'general', //'menu_bar',
			'custom_style' => 'mobile_accordion_indent',
		),
		...
		465 => array(
			'name'		=> 'mobile_modal_close_btn_text',
			'label'		=> __( 'Mobile Modal Close Button Text' , 'ubermenu' ),
			'desc'		=> __( 'Text, HTML or shortcode that will replace the localized "&times; Close" content on the modal close button.' , 'ubermenu' ),
			'type'		=> 'textarea',
			'default'	=> '',
			'group'		=> array( 'responsive' ),
			'sanitize_callback' => 'ubermenu_allow_html',
...
function ubermenu_settings_panel_fields_general( $fields ){
	...
	$fields[UBERMENU_PREFIX.'general'] = array(
		...
		350 => array(
			'name'	=> 'reset_all',
			'label'	=> __( 'Reset All Control Panel Settings' , 'ubermenu' ),
		...
		515 => array(
			'name'	=> 'keyboard_submenu_trigger',
			'label'	=> __( 'Keyboard Submenu Control' , 'ubermenu' ),
			'type'		=> 'radio',
			'options'	=> array(
				'enter'		=> [
					'title' => __( 'Enter' , 'ubermenu' ),
					'desc' => __('First Enter/Return press opens the submenu.  Second Enter/Return press follows the link. ', 'ubermenu' ),
				],
				'spacebar' => [
					'title' => __( 'Spacebar' , 'ubermenu' ),
					'desc' => __('Spacebar toggles the menu open and closed.  Enter/return key will follow the link regardless of submenu state.', 'ubermenu')
				],
				'focus' => [
					'title' => __( 'Focus', 'ubermenu' ),
					'desc' => __('Focusing on/tabbing to a menu item will open its submenu.', 'ubermenu'),
				]
				// 'spacebar'	=> '<div class="ubermenu-op-with-desc"><strong>'.__( 'Spacebar' , 'ubermenu' ).'</strong> <small>'. __('Spacebar toggles the menu open and closed.  Enter/return key will follow the link regardless of submenu state.', 'ubermenu').'</small></div>',
				// 'focus'		=> '<div class="ubermenu-op-with-desc"><strong>'.__( 'Focus', 'ubermenu' ).'</strong> <small>'. __('Focusing on/tabbing to a menu item will open its submenu.', 'ubermenu').'</small></div>',
			),
			'default'	=> 'enter',
			'desc'	=> __( 'Determines how to interact with an item that has a submenu once it is highlighted.', 'ubermenu' ),
						// '<br/>'.__('<strong>Enter</strong> - First Enter opens the submenu.  Second Enter follow the link. ', 'ubermenu' ).
						// '<br/>'.__('<strong>Spacebar</strong> - Spacebar toggles the menu open and closed.  Enter/return key will follow the link regardless of submenu state.', 'ubermenu').
						// '<br/>'.__('<strong>Focus</strong> - Focusing on/tabbing to a menu item will open its submenu.', 'ubermenu'),
			'group'	=> 'accessibility',
		),
		520	=> array(
		...
		571	=> array(
			'name'	=> 'aria_haspopup',
			'label'	=> __( 'aria-haspopup' , 'ubermenu' ),
			'type'	=> 'checkbox',
			'default'	=> 'off',
			'desc'	=> __( 'Add the aria-haspopup attribute to anchors with dropdown submenus', 'ubermenu' ),
			'group'	=> 'accessibility',
		),
		580	=> array(
			'name'	=> 'aria_hidden',
...
add_action( 'admin_init' , 'ubermenu_delete_all_item_settings' , 100 );
function ubermenu_delete_all_item_settings(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'ubermenu-settings' ){
		...
		if( isset( $_GET['do'] ) && $_GET['do'] == 'delete-item-settings' ){

			delete_metadata( 'post', false, UBERMENU_MENU_ITEM_META_KEY, '', true );
			_UBERMENU()->reset_all_item_styles();
			ubermenu_reset_generated_styles();

			//TODO Clear cache
			//TODO Clear UberMenu CSS style cache
...
function ubermenu_control_panel() {
	...
	if( !isset( $_GET['do'] ) ){
		...
		switch( $_GET['do'] ){
			...
			case 'delete-item-settings-check':
				ubermenu_delete_all_menu_item_settings_check_panel();
				break;
			case 'delete-item-settings':
				ubermenu_delete_all_menu_item_settings_complete_panel();
				break;
...
function ubermenu_delete_all_menu_item_settings_check_panel(){

	global $wpdb;
	$table = _get_meta_table( 'post' );
	$query = "SELECT COUNT(*) FROM $table WHERE meta_key = '".UBERMENU_MENU_ITEM_META_KEY."'";
	$count = $wpdb->get_var( $query );

	?>
	<div class="wrap ubermenu-wrap">

		<h2><strong>UberMenu</strong> Menu Item Settings Reset <span class="ubermenu-version">v<?php echo UBERMENU_VERSION; ?></span></h2>

		<?php if( !$count ): ?>
			<p><?php _e('No UberMenu Menu Item Settings found in post meta table.  No action is possible.', 'ubermenu' ); ?></p>
		<?php else: ?>

			<?php
				echo '<div class="error"><p>'.__( 'Please be sure you wish to proceed.  This will delete all UberMenu Menu Item Settings from all menus.  It will not delete menus, menu items, or core WordPress item settings.  This cannot be undone.' , 'ubermenu' ).'</p></div>';

				?>
				<p><?php echo $count; ?> <?php _e( 'menu items found', 'ubermenu' ); ?></p>
				<form action="<?php echo admin_url( 'themes.php' ); ?>" method="GET">
					<input type="hidden" name="page" value="ubermenu-settings" />
					<input type="hidden" name="do" value="delete-item-settings" />
					<?php wp_nonce_field( 'ubermenu-control-panel-do' , 'ubermenu_nonce' ); ?>

					<br/>
					<input type="submit" class="button button-primary" value="<?php _e( 'Confirm &amp; Delete Menu Item Settings' , 'ubermenu' ); ?>" />
				</form>
				<?php
...
function ubermenu_delete_all_menu_item_settings_complete_panel(){
	...
	<div class="wrap ubermenu-wrap">
		...
		<h2><strong>UberMenu</strong> Menu Item Settings Deleted <span class="ubermenu-version">v<?php echo UBERMENU_VERSION; ?></span></h2>

		<?php settings_errors(); ?>

		<?php

			echo '<div class="updated"><p>'.__( 'Menu Item Settings deletion complete!' , 'ubermenu' ).'</p></div>';
...
function ubermenu_settings_panel(){
	...
	echo '<h1><strong>UberMenu</strong> '.__( 'Control Panel', 'ubermenu' ).' <span class="ubermenu-version">v'.UBERMENU_VERSION.'</span></h1>';
...
function ubermenu_settings_links(){
	if( ubermenu_is_pro() ) echo '<a class="button button-quickstart" href="#"><i class="fas fa-bolt"></i> '.__( 'Quick Start', 'ubermenu' ).'</a> ';
	echo '<a target="_blank" class="button button-primary" href="'.UBERMENU_KB_URL.'"><i class="fas fa-book"></i> '.__( 'Knowledgebase', 'ubermenu' ).'</a> ';
	echo '<a target="_blank" class="button button-tertiary" href="'.UBERMENU_VIDEOS_URL.'"><i class="fas fa-video"></i> '.__( 'Video Tutorials', 'ubermenu' ).'</a> ';
	if( ubermenu_is_pro() ) echo '<a target="_blank" class="button button-secondary" href="'.UBERMENU_TROUBLESHOOTER_URL.'"><i class="fas fa-wrench"></i> '.__( 'Troubleshooter', 'ubermenu' ).'</a> ';
	if( ubermenu_is_pro() &&
		( !defined( 'UBERMENU_PACKAGED_THEME' ) || ubermenu_op( 'purchase_code' , 'updates' , '' ) )
		) echo '<a target="_blank" class="button button-secondary ubermenu-button-support" href="'.ubermenu_get_support_url().'"><i class="fas fa-life-ring"></i> '.__( 'Support', 'ubermenu' ).'</a> ';
}
